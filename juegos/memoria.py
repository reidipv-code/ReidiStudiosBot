...
async def memoria(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        partidas_memoria.pop(user_id, None)
        raise
...
async def borrar_secuencia(context) -> None:
    ...
    # No ejecutar temporizadores de una ronda o partida anterior.
    partida = partidas_memoria.get(user_id)
    if partida is None or partida.get("message_id") != message_id:
        return
