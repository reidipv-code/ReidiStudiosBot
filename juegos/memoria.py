...
async def mostrar_secuencia(context, user_id):
    ...
    context.job_queue.run_once(
        ...
        name=f"memoria_secuencia_{user_id}"
    )
...
async def borrar_secuencia(context) -> None:
    ...
    partida_actual = partidas_memoria.get(user_id)
    if partida_actual is None or partida_actual.get("message_id") != message_id:
        return
...
async def terminar_memoria(context, user_id, gano):
    # Cerrar primero la partida evita bloqueos si fallan XP, misiones o mensajes.
    ...
    # Cancelar jobs pendientes de borrado de secuencia de este usuario.
    try:
        jobs = context.job_queue.get_jobs_by_name(f"memoria_secuencia_{user_id}")
        for job in jobs:
